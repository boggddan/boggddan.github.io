...
(function () {
  ...
  var createSVGFilterGrayScale = function createSVGFilterGrayScale(_ref) {
    ...
    feColorMatrix.setAttribute('values', "".concat(1 - filterValue / 100));
  ...
  var createSVGElem = function createSVGElem(_ref2) {
    ...
    var namespaceURI = 'http://www.w3.org/2000/svg'; // Создаем SVG для фона c наложеным фильтром
    ...
    shapeBackground.setAttribute('fill', backgroundColor); // Создаем 2 картинки (обычная и с наложенным фильтром) для анимации фильтра
    ...
    ['filter', 'real'].forEach(function (el) {
      ...
      svg.classList.add("svg-".concat(el));
  ...
  var replaceElem = function replaceElem(el) {
    ...
    var filterId = "filter-grayscale-".concat(filterValue);
    checkFilterExists({
      filterId: filterId,
      filterValue: filterValue
    });
    var container = createSVGElem({
      id: id,
      cssText: cssText,
      classList: classList,
      src: src,
